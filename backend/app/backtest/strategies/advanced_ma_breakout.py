...
class AdvancedMABreakout:
    ...
    def _extreme_thresholds(self, reference_ma: float, sigma: float | None, reference_atr: float | None) -> tuple[float | None, float | None]:
        bias_threshold = None
        atr_threshold = None
        if sigma is not None and not math.isnan(sigma):
            bias_threshold = reference_ma * (1 + self.p.bias_sigma_multiple * sigma)
        if reference_atr is not None and not math.isnan(reference_atr):
    ...
    def process_bar(
        ...
        *,
        ...
        if self.s.current_qty:
            ...
            half_level = reference_ma * (1 - self.p.ma_risk_pct / 100)
            first_target_touched = bool(not self.s.first_tp_triggered and bar.high >= first_target)
            ...
            protective_touched = bool(self.s.first_tp_triggered and bar.low <= protective)
            ...
            newly_activated_protective = bool(first_target_touched and bar.low <= protective)
            break_touched = bool(self.s.break_day_low is not None and self.s.break_day != trading_day and bar.low < self.s.break_day_low)
            half_touched = bool(not self.s.first_tp_triggered and self.s.break_day_low is None and bar.low <= half_level)
        ...
        first_target = (self.s.entry_price or math.inf) * (1 + self.p.first_tp_pct / 100)
        if not self.s.first_tp_triggered and bar.high >= first_target:
            protective = max(self.s.entry_price or 0, reference_ma * (1 - self.p.ma_risk_pct / 100))
            if bar.low <= protective and not self._entry_adverse_preceded and not profit_first_ambiguity:
            ...
            if profit_first_ambiguity and bar.low <= protective and self.s.current_qty:
        ...
        if self.s.current_qty and self.s.first_tp_triggered:
            bias_threshold, atr_threshold = self._extreme_thresholds(reference_ma, bias_sigma, reference_atr)
    ...
    def end_day(
        ...
        *,
        ...
        if self.s.entry_day == trading_day:
            ...
            before = self.s.state
            if previous_day_volume is None or daily_volume < threshold:
                ...
                self.s.day2_validation_pending = True
        ...
        elif self.s.day2_validation_pending:
            before = self.s.state
            self.s.day2_validation_pending = False
        ...
        if self.s.break_day_low is not None and self.s.break_day != trading_day and daily_low > current_day_ma:
            before = self.s.state
