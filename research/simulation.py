...
def simulate(prepared: Prepared, modules: Modules = FULL, *, anchor: dict | None = None,
             ...
             first_tp_fraction: float = .5, horizon_sessions: int | None = None) -> dict:
    ...
    strategy = SimpleMABreakout(req.parameters, rec, execution_policy(policy)) if simple else ResearchAdvanced(req.parameters, rec, execution_policy(policy), modules=modules, first_tp_fraction=first_tp_fraction)
