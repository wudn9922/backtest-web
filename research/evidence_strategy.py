"""PRE-V3 only. Frozen research strategy copy with explicit MA episode gates.
Production and prior frozen research files are never edited/import-patched.
"""
...
class EvidenceAdvanced:
    ...
    """
    def __init__(self, parameters: StrategyParameters, recorder: EventRecorder, execution_policy: DailyIntrabarExecutionPolicy | None = None, modules: Modules = FULL, *, first_tp_fraction: float = .5, ma_mode: str = "full"):
        ...
        if ma_mode not in ('full', 'half_only', 'exit_off'):
            raise ValueError('Unknown frozen MA structural mode')
    ...
    def _start_break(self, trading_day, low):
        """Half-only: at most one half-risk per session, eligible again next session.
        No break tracking/reset. Validation flags survive; post-TP risk unchanged.
        This is a distinct research definition, not a clean isolated causal contrast.
        """
        if self.ma_mode == "half_only":
            self.s.break_day = None
            self.s.break_day_low = None
            self.s.state = (StrategyState.LONG_VALIDATING_DAY1 if self.s.entry_day == trading_day
                            else StrategyState.LONG_VALIDATING_DAY2 if self.s.day2_validation_pending
                            else StrategyState.LONG_NORMAL) if self.s.current_qty else StrategyState.CLOSED
        else:
            self.s.break_day = trading_day
            self.s.break_day_low = low
    ...
    def _handle_entry_day_risk(self, bar: Bar, *, trading_day: date, reference_ma: float, fill: FillCallback) -> bool:
        ...
        self._start_break(trading_day, bar.low)
        self._log(bar, reference_ma, "BREAK_HALF_TRIGGERED", half_level, half_level, risk_before, risk_state)
        self._log(bar, reference_ma, "RESEARCH_HALF_ONLY_REARM_NEXT_SESSION" if self.ma_mode == "half_only" else "BREAK_PROTECTION_STARTED", bar.low, bar.low, self.s.current_qty, self.s.state)
        return True
    ...
    def process_bar(
        ...
        *,
        ...
        if self.s.current_qty:
            ...
            break_touched = bool(self.ma_mode != 'exit_off' and self.modules.ma_break and (self.modules.protective or not self.s.first_tp_triggered) and self.s.break_day_low is not None and self.s.break_day != trading_day and bar.low < self.s.break_day_low)
